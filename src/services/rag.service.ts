...
//     model: CLAUDE_MODEL,
//     max_tokens: 1024,
//     system: `Você é um engenheiro elétrico especializado em relés de proteção. Responda APENAS com base no contexto do manual fornecido. Se a resposta não estiver no contexto, diga "Não encontrei essa informação no manual."`,
//     messages: [
//       {
//         role: "user",
//         content: `Contexto do manual:\n${context}\n\nPergunta: ${question}`,
//       },
//     ],
//   });
...
export async function answerQuestion(
  ...
  // 4. Chamar Groq (LLaMA 3)
  const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });

  const completion = await groq.chat.completions.create({
    model: "llama-3.3-70b-versatile", // ou 'llama-3.1-8b-instant' para versão mais recente
    max_tokens: 1024,
    messages: [
      {
        role: "system",
        content: `Você é um engenheiro elétrico especializado em relés de proteção. Responda APENAS com base no contexto do manual fornecido. Se a resposta não estiver no contexto, diga "Não encontrei essa informação no manual."`,
      },
      {
        role: "user",
        content: `Contexto do manual:\n${context}\n\nPergunta: ${question}`,
...
export async function compareAcrossMachines(
  ...
  const completion = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `Você é um assistente técnico especializado. Compare as informações das máquinas com base nos manuais fornecidos. Se alguma máquina não tiver dados, mencione isso na resposta.`,
      },
